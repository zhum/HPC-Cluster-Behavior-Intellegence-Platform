...
The cache is a bounded LRU (CACHE_MAX_ENTRIES, least-recently-used evicted)
so long interactive sessions sweeping many parameter combinations don't grow
memory without limit. Redis-backed cross-process caching is added in Phase 5
(analysis-api); this is the in-process half of that design.
"""
...
CACHE_MAX_ENTRIES = 32  # per-stage results; oldest-used evicted beyond this
...
class InterClusterPipeline:
    ...
    def get_V(self) -> np.ndarray:
        key = _key("V", {})
        return self._cached(key, lambda: dr1_pca_over_time(self.X))
    ...
    def get_E(self, n_neighbors: int = 15, min_dist: float = 0.1, random_state: int = 42) -> np.ndarray:
        ...
        return self._cached(
            key, lambda: dr2_umap(V, n_neighbors=n_neighbors, min_dist=min_dist, random_state=random_state)
        )
    ...
    def get_labels(
        ...
        return self._cached(key, lambda: kmeans_cluster(E, k=k, random_state=random_state, n_init=n_init))
    ...
    def get_ccpca(
        ...
        return self._cached(key, lambda: ccpca_explain(V, labels))
